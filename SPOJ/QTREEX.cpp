...
struct node{
    int maxV, minV, flag;
    node(){}
    node(int _maxV, int _minV, int _flag){
        maxV = _maxV; minV = _minV; flag = _flag;
    }
};
int n, a, b, ind;
edge e[N];
...
void build(int pos, int l, int r) {
    ...
    tree[pos].maxV = max(tree[c].maxV, tree[c + 1].maxV);
    tree[pos].minV = min(tree[c].minV, tree[c + 1].minV);
    tree[pos].flag = 0;
}
void edit(int pos) {
    int temp = tree[pos].minV;
    tree[pos].minV = -tree[pos].maxV;
    tree[pos].maxV = -temp;
}
...
int get(int pos, int u, int v, int l, int r) {
    ...
    if (u <= l && r <= v) return tree[pos].maxV;
    int mid = (l + r) >> 1, c = 2 * pos;
    if ( tree[pos].flag ) {
        edit(c); edit(c + 1);
        tree[c].flag ^= 1;
...
void reverseSign(int pos, int u, int v, int l, int r) {
    if (r < u || l > v) return;
    if (u <= l && r <= v) {
        tree[pos].flag ^= 1;
        edit(pos);
    ...
    reverseSign(c, u, v, l, mid);
    reverseSign(c + 1, u, v, mid + 1, r);
    tree[pos].maxV = max(tree[c].maxV, tree[c + 1].maxV);
    tree[pos].minV = min(tree[c].minV, tree[c + 1].minV);
}
...
void solveQ2(int u, int v) {
    while (u != -1 && head[u] != head[v] ) {
        ...
        reverseSign(1, number[head[u]], number[u], 1, n);
        //u = (heavy[u] ? head[u] : parent[u]);
        u = parent[head[u]];
    }
    if (depth[u] > depth[v]) swap(u, v);
    if (u != v)
        reverseSign(1, number[u] + 1, number[v], 1, n);
}
void solveQ3(int u, int v) {
    int res = -(1 << 30);
    while (u != -1 && head[u] != head[v] ) {
        if (depth[head[u]] < depth[head[v]]) swap(u, v);
...
int main() {
	...
	while (cases--) {
	    ...
	    //fr(i, 1, n) cout <<i<<" "<<child[i] << endl;
	    //fr(i, 1, n) cout <<i<<" "<<number[i]<<endl;
	    fr(i, 1, n - 1) {
