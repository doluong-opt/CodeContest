...
ii d[N];
int n, m, q, lf, rg;
int a[N], b[N], c[N], key[N], f[N];
int markB[N], ans;
vector <ii> iter;
set <int> sA;
set <int> :: iterator it;
int fl_(int x, int l, int r) {
    while (l <= r) {
        int mid = (l + r) >> 1;        
        if ( c[mid] >= x ) r = mid - 1;
        else l = mid + 1;
...
int main() {
    ...
    rep(i, iter.size()) {
        ...
        int cnt = 0;
        rep(j, n) {
            int x = fl_(key[j], iter[i].ff, iter[i].ss);
            int y = fr_(key[j], iter[i].ff, iter[i].ss);
            if (c[x] == key[j] && x <= y && y - x + 1 >= f[c[x]]) cnt++;
            else break;
        }
