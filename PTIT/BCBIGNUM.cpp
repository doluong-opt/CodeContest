...
bool cmp(string x, string y) {
    ...
    if (id < nx && x[id] < y[id]) return false;
    return true;
}
int main() {
	...
	sum.add(0); sum += A; sum += B;
	sub.add(0); sub += A; sub -= B;
	multi.add(1); multi *= A; multi *= B;
	sum.output(); if (sign) printf("-");sub.output(); multi.output();
	return 0;
}
