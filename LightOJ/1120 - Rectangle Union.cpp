...
void solve() {
    ...
    //cout<<tree[1].sum<<" "<<tree[2].sum<<" "<<tree[3].sum<<" "<<tree[4].sum<<" "<<tree[5].sum<<endl;
    //cout<<tree[1].value<<" "<<tree[2].value<<" "<<tree[3].value<<" "<<tree[4].value<<" "<<tree[5].value<<endl;
    for (int i=1;i<edges.size();i++) {
        S+=tree[1].sum * (edges[i].x - edges[i-1].x);        
        //cout<<S<<" "<<i<<" "<<edges[i].begin<<" "<<edges[i].ymin<<" "<<edges[i].ymax<<endl;
        if (edges[i].begin)
...
int main(){
    ...
    #endif
    int cases = 0, n, caseno = 0;
    int x1, y1, x2, y2, l;
    gets(s);
    for (int i=0;s[i];i++) cases = cases * 10 + s[i] - '0';
    while (cases--) {
        gets(s); n = 0;
